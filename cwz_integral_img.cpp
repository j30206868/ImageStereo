...
template <class I, class O> inline void normal_to_gray_img(I *n_arr, O *arr, int len, int max_intensity){
	for(int i=0; i<len; i++){ 
		double tmp = n_arr[i] * max_intensity;
	
		if(tmp < 0)
			arr[i] = 0;
		else if(tmp > max_intensity)
			arr[i] = max_intensity;
		else
			arr[i] = cvRound(tmp);
	}
...
template <class N, class I, class O> void apply_gray_guided_img_filtering(uchar *img, int h, int w, N *nor_img, guided_img<I, O> gfilter){
	...
	/*gfilter.img_i = nor_img;
	gfilter.img_p = nor_img;
	RESULT_TYPE *result = gfilter.filtering();*/
	RESULT_TYPE *result = gfilter.filtering_with_self(nor_img);
	normal_to_gray_img<RESULT_TYPE, uchar>(result, img, w*h);
}
...
template<class T, class P>
RESULT_TYPE *guided_img<T, P>::filtering_with_self(T *_img_i){
	if(filter_result == NULL)
		filter_result = new RESULT_TYPE[w * h];
	this->img_i = _img_i;
	this->img_p = _img_i;
	...
	//get a = cov_ip(=var_i) / (  var_i + epsilon  )
	for(int i=0 ; i<len ; i++){  mean_a[i] = var_i[i] / (var_i[i] + epsilon); }
	//get b = mean_p(=mean_i) - a * mean_i
	for(int i=0 ; i<len ; i++){  mean_b[i] = mean_i[i] - mean_a[i] * mean_i[i];  }
