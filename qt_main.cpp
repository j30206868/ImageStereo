...
//sub sampling and producing sub sampled depth map
    ...
    do{
        ...
        {   //update variable value from global config
            ...
            show_cv_img("Disparity Image", refined_dmap, left.rows, left.cols, 1, false);
...
void show_img_diff_with_former(cv::Mat &lastLm, cv::Mat &lastRm, cv::Mat &left, cv::Mat &right, match_info &info){
    ...
    show_cv_img("Variation of Left Image", diff_gray_left, diffLm.rows, diffLm.cols, 3, false);
    show_cv_img("Variation of Right Image", diff_gray_right, diffRm.rows, diffRm.cols, 3, false);
}
